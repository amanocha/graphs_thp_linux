 ...
 *
 *  MADV_IRREG - mark the given range as irregularly accessed, so a more 
...
{
        //printk(KERN_DEBUG "do_madvise: called with behavior %d!\n", behavior); //ANINDA
	...
	//if (behavior == MADV_IRREG) printk(KERN_DEBUG "MADV_IRREG: start = %lu, end = %lu, len_in = %lu, len = %lu\n", start, end, len_in, len); //ANINDA
		...
		size_t, vlen, int, behavior, unsigned int, flags)
{
	...
	printk(KERN_DEBUG "process_madvise: pidfd = %d, vlen = %lu, behavior = %d\n", pidfd, vlen, behavior);
	...
	pid_val = pid->numbers[0].nr; //ANINDA
	if (IS_ERR(pid)) {
		ret = PTR_ERR(pid);
		goto free_iov;
	}
	...
	while (iov_iter_count(&iter)) {
		...
		// ANINDA: adding support for synchronous promotion and demotion
		vaddr_start = (unsigned long)iovec.iov_base;
		vaddr_end = vaddr_start + iovec.iov_len;
